...
by a unique substring of its headline, so no URL is typed by hand. Paid For's is
`scripts/build-paid-for.cjs`: edit it and regenerate rather than hand-editing the JSON. (Press-ban's
data was edited directly after it was generated, so it has no current script.)
